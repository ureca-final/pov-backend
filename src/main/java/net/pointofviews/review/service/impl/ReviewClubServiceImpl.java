...
public class ReviewClubServiceImpl implements ReviewClubService {
    ...
    @Override
    public ReadMyClubReviewListResponse findReviewByClub(UUID clubId, Pageable pageable) {

        if (clubRepository.findById(clubId).isEmpty()) {
            throw clubNotFound(clubId);
        }

        Slice<ReadReviewResponse> reviews = memberClubRepository.findReviewsWithLikesByClubId(clubId, pageable);
